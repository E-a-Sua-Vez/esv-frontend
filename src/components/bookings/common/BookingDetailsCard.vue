...
export default {
  ...
  components: { Popper, Spinner, Warning, AreYouSure },
  props: {
    ...
    selectPaymentType($event) {
      if ($event && $event.target) {
        ...
        if (['PAID', 'EVALUATION', 'PROMOTION', 'TRIAL'].includes(paymentType)) {
          ...
          this.newConfirmationData.paymentMethod = ''
          this.newConfirmationData.paymentAmount = null;
          this.newConfirmationData.paymentCommission = null
        }
      }
    }
  },
            ...
            <div
              ...
              <div id="payment-type-form-add" class="row g-1 my-1">
                ...
                <div class="col-8">
                  <select
                    ...
                    v-model="newConfirmationData.paymentType"
                    id="types"
                    @change="selectPaymentType($event)"
                    v-bind:class="{ 'is-invalid': paymentAmountError }">
                    <option v-for="typ in paymentTypes" :key="typ.name" :value="typ.id">{{ $t(`paymentTypes.${typ.name}`) }}</option>
                  </select>
                </div>
              </div>
              <div id="payment-method-form-add" class="row g-1 my-1">
                ...
                <div class="col-8">
                  <select
                    class="btn btn-md btn-light fw-bold text-dark select"
                    v-model="newConfirmationData.paymentMethod"
                    id="types"
                    v-bind:class="{ 'is-invalid': paymentMethodError }">
                    <option v-for="typ in paymentMethods" :key="typ.name" :value="typ.id">{{ $t(`paymentClientMethods.${typ.name}`) }}</option>
                  </select>
                </div>
              </div>
              <div id="payment-amount-form-add" class="row g-1 my-1">
                ...
                <div class="col-8">
                  <input
                    ...
                    type="number"
                    class="form-control"
                    v-model="newConfirmationData.paymentAmount"
                    v-bind:class="{ 'is-invalid': paymentAmountError }"
                    placeholder="100">
                </div>
              </div>
              <div id="payment-commission-form-add" class="row g-1 my-1">
                ...
                <div class="col-8">
                  <input
                    ...
                    type="number"
                    class="form-control"
                    v-model="newConfirmationData.paymentCommission"
                    placeholder="100">
                </div>
              </div>
              <div id="payment-comment-form-add" class="row g-1 mt-1">
                  <textarea
                    ...
                    id="comment"
                    rows="2"
                    v-model="newConfirmationData.paymentComment"
