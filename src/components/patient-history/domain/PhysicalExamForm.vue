...
export default {
  ...
  async setup(props) {
    ...
    onBeforeMount(async () => {
      try {
        ...
        if (patientHistoryData.value && patientHistoryData.value.id) {
          state.oldPhysicalExam = patientHistoryData.value.physicalExam;
        }
        if (cacheData.value) {
    ...
    const checkAsc = (event) => {
      ...
      if (state.oldPhysicalExam && state.oldPhysicalExam.length > 0) {
        let elementsSorted = [];
        const elements = state.oldPhysicalExam;
        if (state.asc) {
        ...
        state.oldPhysicalExam = elementsSorted;
      }
    }
    ...
    <div id="form">
      <div class="row">
        ...
        <div class="col-12 col-md-6 mt-2 blocks-section">
          ...
          <div v-if="state.oldPhysicalExam && state.oldPhysicalExam.length > 0 && state.oldPhysicalExam[0]">
            <div v-for="(element, index) in state.oldPhysicalExam" :key="`exam-${index}`">
              <HistoryDetailsCard
