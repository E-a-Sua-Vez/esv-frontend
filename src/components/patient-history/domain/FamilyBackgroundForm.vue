...
export default {
  ...
  async setup(props) {
    ...
    onBeforeMount(async () => {
      try {
        ...
        if (patientHistoryData.value && patientHistoryData.value.id) {
          state.oldFamilyBackground = patientHistoryData.value.familyBackground;
        }
        if (cacheData.value) {
          state.newFamilyBackground = cacheData.value;
        }
        loading.value = false;
      } catch (error) {
        loading.value = false;
    ...
    const sendData = () => {
      receiveData(state.newFamilyBackground);
    }
    ...
    const checkAsc = (event) => {
      ...
      if (state.oldFamilyBackground && state.oldFamilyBackground.length > 0) {
        let elementsSorted = [];
        const elements = state.oldFamilyBackground;
        if (state.asc) {
        ...
        state.oldFamilyBackground = elementsSorted;
      }
    }
    ...
    watch (
      ...
      async () => {
        loading.value = true;
        if (patientHistoryData.value && patientHistoryData.value.id) {
          if (patientHistoryData.value.familyBackground &&
            patientHistoryData.value.familyBackground.length > 0 &&
            patientHistoryData.value.familyBackground[0]
          )
          state.oldFamilyBackground = patientHistoryData.value.familyBackground;
        }
        loading.value = false;
    ...
    <div id="form">
      <div class="row">
        <div class="col-12 col-md-6 mt-2">
          <div id="patient-name-form-add" class="row m-1">
            <div class="col-12 text-label">
              {{ $t("patientHistoryView.familyBackground") }} <i class="bi bi-people-fill mx-1"></i>
            </div>
            <div class="col-12">
              <textarea
                ...
                v-model="state.newFamilyBackground.background">
              </textarea>
            </div>
          </div>
        ...
        <div class="col-12 col-md-6 mt-2 blocks-section">
          ...
          <div v-if="state.oldFamilyBackground && state.oldFamilyBackground.length > 0 && state.oldFamilyBackground[0]">
            <div v-for="(element, index) in state.oldFamilyBackground" :key="`background-${index}`">
              <HistoryDetailsCard
