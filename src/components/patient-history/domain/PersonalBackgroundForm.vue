...
export default {
  ...
  async setup(props) {
    ...
    const state = reactive({
      newPersonalBackground: {},
      oldPersonalBackground: [],
      captcha: false,
    ...
    onBeforeMount(async () => {
      try {
        ...
        if (patientHistoryData.value && patientHistoryData.value.id) {
          state.oldPersonalBackground = patientHistoryData.value.personalBackground;
        }
        if (cacheData.value) {
          state.newPersonalBackground = cacheData.value;
        }
        loading.value = false;
      } catch (error) {
        loading.value = false;
    ...
    const sendData = () => {
      receiveData(state.newPersonalBackground);
    }
    ...
    const checkAsc = (event) => {
      ...
      if (state.oldPersonalBackground && state.oldPersonalBackground.length > 0) {
        let elementsSorted = [];
        const elements = state.oldPersonalBackground;
        if (state.asc) {
        ...
        state.oldPersonalBackground = elementsSorted;
      }
    }
    ...
    watch (
      ...
      async () => {
        loading.value = true;
        if (patientHistoryData.value && patientHistoryData.value.id) {
          if (patientHistoryData.value.personalBackground &&
            patientHistoryData.value.personalBackground.length > 0 &&
            patientHistoryData.value.personalBackground[0]
          )
          state.oldPersonalBackground = patientHistoryData.value.personalBackground;
        }
        loading.value = false;
    ...
    <div id="form">
      <div class="row">
        <div class="col-12 col-md-6 mt-2">
          <div id="patient-name-form-add" class="row m-1">
            <div class="col-12 text-label">
              <span>{{ $t("patientHistoryView.personalBackground") }} </span> <i class="bi bi-person-fill mx-1"></i>
            </div>
            <div class="col-12">
              <textarea
        ...
        <div class="col-12 col-md-6 mt-2 blocks-section">
          ...
          <div v-if="state.oldPersonalBackground && state.oldPersonalBackground.length > 0 && state.oldPersonalBackground[0]">
            <div v-for="(element, index) in state.oldPersonalBackground" :key="`background-${index}`">
              <HistoryDetailsCard
