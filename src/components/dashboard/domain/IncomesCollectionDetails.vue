...
export default {
  ...
  methods: {
    ...
    distributionPaymentPercentage(total, tag) {
      return parseFloat(((this.distributionPayment[tag].count * 100) / total).toFixed(2), 2) || 0;
    },
    ...
    distributionFiscalNotePercentage(total, tag) {
      return (
        parseFloat(((this.distributionFiscalNote[tag].count * 100) / total).toFixed(2), 2) || 0
      );
    },
    classifyIncomeStatus(status) {
      if (status === 'CONFIRMED') {
        return 'bg-success';
    ...
    <div v-if="count">
      <div class="notification-grid">
        ...
        <div class="notification-item">
          ...
          <div class="notification-content">
            <div class="notification-label">{{ $t('dashboard.incomes') }}</div>
        ...
        <div class="notification-item">
          ...
          <div class="notification-content">
            <div class="notification-label">{{ $t('dashboard.commissions') }}</div>
            <div class="notification-value">
              {{
                distribution.paymentCommissionSum
                  ? Number(distribution.paymentCommissionSum).toLocaleString('de-DE')
                  : 0
              }}
            </div>
          </div>
      ...
      <div v-if="showDetailsSection" class="details-expandable-section">
        ...
        <Transition name="details-expand">
          <div v-if="extendedEntity" class="detailed-data">
            <div v-if="Object.keys(distributionPayment).length > 0" class="distribution-section">
              ...
              <div class="distribution-items">
                ...
                >
                  ...
                  <span class="distribution-item-badge" :class="classifyIncomeStatus(origin)">
              ...
              <div class="distribution-items">
                ...
                >
                  <div class="distribution-item-header">
                    <span class="distribution-item-label">{{ $t(`incomeTypes.${origin}`) }}</span>
            ...
            <div v-if="Object.keys(distributionMethod).length > 0" class="distribution-section">
              <h6 class="distribution-section-title">
                {{ $t('dashboard.paymentMethod') }}
              </h6>
              <div class="distribution-items">
                <div
                  v-for="origin in Object.keys(distributionMethod)"
                  :key="origin"
                  class="distribution-item-with-progress"
                >
                  <div class="distribution-item-header">
                    <span class="distribution-item-label">{{
                      $t(`paymentClientMethods.${origin}`)
                    }}</span>
                    <span class="distribution-item-badge-secondary">
                      {{ distributionMethodPercentage(count, origin) }}%
                    </span>
                  </div>
                  <div class="modern-progress-bar-small">
                    <div
                      class="progress-segment progress-segment-blue"
                      :style="`width: ${distributionMethodPercentage(count, origin)}%`"
                    >
                      <span class="progress-segment-value-small">
                        {{ distributionMethod[origin].count || 'N/I' }}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div v-if="Object.keys(distributionFiscalNote).length > 0" class="distribution-section">
              <h6 class="distribution-section-title">
                {{ $t('dashboard.paymentFiscalNote') }}
              </h6>
              <div class="distribution-items">
                <div
                  v-for="origin in Object.keys(distributionFiscalNote)"
                  :key="origin"
                  class="distribution-item-with-progress"
                >
                  <div class="distribution-item-header">
                    <span class="distribution-item-label">{{
                      $t(`paymentFiscalNotes.${origin}`)
                    }}</span>
                    <span class="distribution-item-badge-secondary">
                      {{ distributionFiscalNotePercentage(count, origin) }}%
                    </span>
                  </div>
                  <div class="modern-progress-bar-small">
                    <div
                      class="progress-segment progress-segment-blue"
                      :style="`width: ${distributionFiscalNotePercentage(count, origin)}%`"
                    >
                      <span class="progress-segment-value-small">
                        {{ distributionFiscalNote[origin].count || 'N/I' }}
                      </span>
                    </div>
                  </div>
                </div>
