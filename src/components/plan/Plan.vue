...
<template>
  <div v-if="plan.active" class="m-2">
    <div :class="`card card-${getPlanName(plan)} ${selectionable ? 'card-selectionable' : ''} ${plan.id === selectedPlan.id && selectionable ? 'card-selected' : ''}`">
      <div :class="`mt-1 plan-title plan-title-${plan.name.toLowerCase()}`">
        <span class="details"><i :class="`m-1 bi ${getPlanIcon(plan)}`"></i></span><span class="plan">{{ $t("businessPlan.plan") }}</span><span class="details fw-bold">{{ plan.name }}</span>
      </div>
      <div v-if="plan.value > 0" class="mt-1 plan-price">
        <span class="plan-amount details fw-bold"><i class="bi bi-coin"></i> {{ plan.currency }}{{ plan.value }}</span> /{{ plan.periodicity }}
      </div>
      <div v-else class="mt-2 plan-price">
        <span class="plan-amount details fw-bold"><i class="bi bi-coin"></i> {{ $t("businessPlan.consult") }}</span>
      </div>
      <div class="card-body">
