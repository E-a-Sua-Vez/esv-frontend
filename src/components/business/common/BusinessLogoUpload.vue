  ...
  setup(props, { emit }) {
    ...
    const handleFileUpload = event => {
      ...
      if (!supportedFormats.includes(file.type)) {
        alert(t('businessAdmin.logoUpload.errors.fileFormatNotSupported'));
        return;
      }

      // Validate file size
      if (file.size > maxFileSize) {
        alert(t('businessAdmin.logoUpload.errors.fileTooLarge'));
        return;
      }
      ...
      reader.onload = e => {
        uploadedLogo.value = {
          file,
          url: e.target.result,
          filename: `business-${props.businessId}-${Date.now()}-${file.name}`,
        };
      };
    ...
    const saveLogo = async () => {
      ...
      try {
        ...
        alert(t('businessAdmin.logoUpload.errors.saveFailed'));
      }
    };
    ...
    <div class="logo-upload-container">
      ...
      <div class="logo-upload-header">
        <h4 class="logo-upload-title">
          ...
          {{ $t('businessAdmin.logoUpload.title') }}
        </h4>
      ...
      <div class="logo-upload-content">
        ...
        <div v-if="existingLogoUrl && !uploadedLogo" class="existing-logo-preview">
          <h5 class="preview-title">{{ $t('businessAdmin.logoUpload.currentLogo') }}</h5>
          <div class="preview-container">
            <img
              :src="existingLogoUrl"
              :alt="$t('businessAdmin.logoUpload.currentLogo')"
              class="preview-image"
            />
          </div>
          <p class="preview-hint">{{ $t('businessAdmin.logoUpload.replaceHint') }}</p>
        ...
        <div v-if="showUpload && !uploadedLogo && !existingLogoUrl" class="upload-container">
          <div class="upload-area" @click="$refs.fileInputRef.click()">
            ...
            <h5>{{ $t('businessAdmin.logoUpload.selectLogo') }}</h5>
            <p>{{ $t('businessAdmin.logoUpload.fileFormats') }}</p>
            <p class="upload-hint">{{ $t('businessAdmin.logoUpload.recommendedSize') }}</p>
            <input
              ref="fileInputRef"
              type="file"
        ...
        <div v-if="showUpload && !uploadedLogo && existingLogoUrl" class="upload-container">
          <div class="upload-area" @click="$refs.fileInputRef.click()">
            <i class="bi bi-cloud-upload upload-icon"></i>
            <h5>{{ $t('businessAdmin.logoUpload.selectNewLogo') }}</h5>
            <p>{{ $t('businessAdmin.logoUpload.fileFormats') }}</p>
            <input
              ref="fileInputRef"
              type="file"
        ...
        <div v-if="uploadedLogo" class="logo-preview">
          <div class="preview-container">
            <img
              ...
              :alt="$t('businessAdmin.logoUpload.preview')"
              class="preview-image"
            />
          </div>
          ...
          <div class="preview-actions">
            <button class="btn-save" @click="saveLogo">
              ...
              {{ $t('businessAdmin.logoUpload.saveLogo') }}
            </button>

            <button class="btn-retake" @click="retakeLogo">
              <i class="bi bi-arrow-clockwise me-1"></i>
              {{ $t('businessAdmin.logoUpload.retake') }}
            </button>
          </div>
        </div>
      </div>
