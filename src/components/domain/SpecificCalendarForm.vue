...
export default {
  ...
  async setup(props) {
    ...
    const { show, locale, structure } = toRefs(props);
    ...
    watch(changeSelectedMonth, async () => {});
    ...
    <div class="mb-2">
      <div class="row">
        <div class="my-2 selected-days-title">
          <span class="selected-days-title">
            {{ $t('businessCommercesAdmin.selectSpecificDate') }}
          </span>
        </div>
        ...
        <div class="col-12 col-md-6 mt-2">
          <div class="my-1 selected-days-title">
            <span class="selected-days-title text-label">
              {{ $t('businessCommercesAdmin.selectedDate') }}
            </span>
          </div>
          <div class="col-12">
            <span class="badge bg-primary my-1 p-2"
              >{{ getDate(new Date(state.selectedDate)) }}
            </span>
          </div>
          <div class="my-1 selected-days-title">
            <span class="selected-days-title text-label">
              {{ $t('businessCommercesAdmin.hours') }}
            </span>
          </div>
          <div class="row">
            <div class="col-5">
          ...
          <div class="centered">
            <nav>
              <ul class="pagination pagination-ul">
                ...
                <li>
                  <select
                    class="btn btn-sm btn-light fw-bold text-dark select mx-1 py-1"
                    v-model="state.page"
          ...
          <div v-if="state.filteredDates && state.filteredDates.length > 0">
            <div class="row centered my-1 mx-1" v-for="date in state.filteredDates" :key="date">
              ...
              <div class="col-5 selected-days-title">
                {{
                  timeConvert(structure.serviceInfo.specificCalendarDays[date].attentionHourFrom)
                }}
                -
                {{ timeConvert(structure.serviceInfo.specificCalendarDays[date].attentionHourTo) }}
              </div>
              <div class="col-3">
                <button
