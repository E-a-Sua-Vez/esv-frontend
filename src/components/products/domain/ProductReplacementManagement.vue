...
export default {
  ...
  methods: {
    ...
    async refresh() {
      try {
        this.loading = true;
        this.newProductReplacements = await getProductsReplacementDetails(
    ...
    validateAdd(newProductReplacement) {
      this.errorsAdd = [];
      if (
        !newProductReplacement.replacementAmount ||
        newProductReplacement.replacementAmount <= 0
      ) {
      ...
      if (
        !newProductReplacement.replacementDate ||
        newProductReplacement.replacementDate.length === 0
      ) {
    ...
    async exportToCSV() {
      try {
        this.loading = true;
        let csvAsBlob = [];
        const result = await await getProductsReplacementDetails(
          this.product.productId,
          undefined,
          undefined,
          this.asc,
