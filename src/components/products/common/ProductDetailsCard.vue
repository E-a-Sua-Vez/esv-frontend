      ...
      <div class="lefted">
        <div class="progress col-12 col-md-6" style="height: 3px">
          <div
            ...
            role="progressbar"
            :style="`height: 3px; width: ${scorePercentage(
              product.maximumLevel,
              product.actualLevel ? product.actualLevel : 0
            )}%`"
            aria-valuemin="0"
      ...
      <div :class="{ show: extendedEntity }" class="detailed-data transition-slow">
        <div class="row m-0">
          <div class="d-block col-12 col-md-6">
            <div class="my-1">
              <div class="row bar-label">
                <span class="col lefted m-1">
                  <i class="bi bi-caret-down-fill"></i> {{ product.minimumLevel || 0 }}</span
                >
                <span class="col centered m-1 badge bg-dark"> {{ product.actualLevel || 0 }}</span>
                <span class="col righted m-1">
                  {{ product.maximumLevel }} <i class="bi bi-caret-down-fill"></i
                ></span>
              </div>
              <div class="progress" style="height: 20px">
                <div
                  :class="`progress-bar ${productScoreBarStyle(product)}`"
                  role="progressbar"
                  :style="`height: 20px; width: ${scorePercentage(
                    product.maximumLevel,
                    product.actualLevel ? product.actualLevel : 0
                  )}%`"
                  aria-valuemin="0"
          ...
          <div class="col-12 col-md-6 my-2">
            <div class="centered">
              <span class="fw-bold mx-1"> {{ $t('businessProductStockAdmin.actualLevel') }} </span>
              {{ product.actualLevel }}
              {{ $t(`productMeasuresTypesShort.${product.productMeasureType}`) }}
            </div>
            <div class="centered">
              <span class="fw-bold mx-1"> {{ $t('businessProductStockAdmin.optimumLevel') }} </span>
              {{ product.optimumLevel }}
              {{ $t(`productMeasuresTypesShort.${product.productMeasureType}`) }}
            </div>
            <div class="centered">
        ...
        <div class="row centered my-2" v-if="management && !loading">
          <div class="col-6">
            ...
            >
              {{ $t('businessProductStockAdmin.consuptions') }} <br />
          ...
          <div class="col-6">
            ...
            >
              {{ $t('businessProductStockAdmin.replacements') }} <br />
            ...
            <div v-if="product.lastReplacementAmount" class="col">
              <div class="mb-2">
                <span v-if="product.lastReplacementAmount" class="mx-1">
                  <i class="bi bi bi-arrow-down-circle-fill mb-2"></i>
                  {{ $t('businessProductStockAdmin.lastReplacement') }}
                </span>
              </div>
              <span
                v-if="product.lastReplacementAmount"
                class="badge mx-1 detail-data-badge bg-warning"
              >
                <span class="fw-bold detail-data-badge-title">
                  {{ $t('paymentData.productQuantity') }}
                </span>
                <i class="bi bi-eyedropper mx-1"></i> {{ product.lastReplacementAmount }}
                {{ $t(`productMeasuresTypesShort.${product.productMeasureType}`) }}
              </span>
              <span v-if="product.lastReplacementDate" class="badge mx-1 detail-data-badge">
                <span class="fw-bold detail-data-badge-title">
                  {{ $t('paymentData.lastReplacementDate') }}
                </span>
                {{ product.lastReplacementDate }}</span
              >
              <span
                v-if="product.lastReplacementExpirationDate"
                class="badge mx-1 detail-data-badge"
              >
                <span class="fw-bold detail-data-badge-title">
                  {{ $t('paymentData.productExpiration') }}
                </span>
                <i class="bi bi-heart-pulse-fill mx-1"> </i>
                {{ product.lastReplacementExpirationDate }}</span
              >
              <span v-if="product.nextReplacementDate" class="badge mx-1 detail-data-badge">
                <span class="fw-bold detail-data-badge-title">
                  {{ $t('paymentData.productNext') }}
                </span>
              ...
              <span v-if="product.lastReplacementBy" class="badge mx-1 detail-data-badge">
                ...
                <i class="bi bi-person-fill mx-1"> </i> {{ product.lastReplacementBy }}</span
              ><br />
            </div>
          </div>
        </div>
          ...
          <div class="col">
            <div class="mt-2">
              <span class="metric-card-details mx-1"
                ><strong>Id:</strong> {{ product.productId }} <strong>Code:</strong>
        ...
        <div class="modal-content">
          <div class="modal-header border-0 centered active-name">
            <h5 class="modal-title fw-bold">
              <i class="bi bi-arrow-up-circle-fill"></i>
              {{ $t('businessProductStockAdmin.consuptionsOf') }} {{ product.productName }}
            </h5>
            <button
              class="btn-close"
              type="button"
      ...
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header border-0 centered active-name">
            <h5 class="modal-title fw-bold">
              <i class="bi bi-arrow-down-circle-fill"></i>
              {{ $t('businessProductStockAdmin.replacementsOf') }} {{ this.product.productName }}
            </h5>
            <button
              class="btn-close"
              type="button"
